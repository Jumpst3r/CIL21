...
class soft_cldice(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        ax1.imshow(np.array(skel_pred.detach().cpu().numpy()[0,0,:,:]), cmap='binary_r')
        ax2.imshow(np.array(skel_true.detach().cpu().numpy()[0,0,:,:]), cmap='binary_r')
        ax3.imshow(np.array(y_true.detach().cpu().numpy()[0,0,:,:]), cmap='binary_r')
        plt.show()
        '''
