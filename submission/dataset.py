...
# This is the code we used to crop 400x400 patches from the deepglobe dataset.
# We have included the resulting crops so this code is just here for reference
def dataset_transform():
    ...
    for _ in range(5000):
        for im_n, gt_n in zip(images_names, gt_name):
            ...
            gt = torch.tensor(np.where(np.array(pil_gt) >= 1, 1.,
                                       0.)).unsqueeze(0)
            transform = transforms.RandomResizedCrop(400,
                                                     scale=(0.1, 0.2),
                                                     ratio=(1., 1.))
            ...
            npim = Image.fromarray(
                np.array(tf(image=np.array(npim, dtype=np.uint8))['image'],
                         dtype=np.uint8))
