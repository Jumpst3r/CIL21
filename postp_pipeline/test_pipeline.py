...
def infer_test_augment(dataset, model):
    ...
    for batch, idx in dataset:
        ...
        y1 = torch.sigmoid(model(torch.flip(img0, [2])).squeeze(0))
        y2 = torch.sigmoid(model(torch.flip(img0, [3])).squeeze(0))
        y = torch.cat((y0, y1, y2))
        for j in range(1, 4):
            imgr = torch.rot90(img0, j, [2,3])
            imgrr = torch.sigmoid(model(imgr).squeeze(0))
            imgrf = torch.sigmoid(model(imgr).squeeze(0))
            imgrh = torch.sigmoid(model(imgr).squeeze(0))
