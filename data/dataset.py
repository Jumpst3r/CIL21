...
class ArealDataset(Dataset):
    ...
    '''
    def __init__(self, root_dir_images: str, root_dir_gt: str, target_size=(608,608), visualize=False,
                 means=None, stds=None, val=False):
        ...
        if means == None or stds == None:
            ...
            for imname in impaths:
                ...
                count += 1
            self.means = list(np.array(means).sum(axis=0) / count)
            self.stds = list(np.array(stds).sum(axis=0) / count)
        else:
            self.means = means
            self.stds = stds
    ...
    def __getitem__(self, idx):
        ...
        if self.visualize:
                ...
                plt.show()
        # One hot encoded mask, use with binary cross entropy and a one unit output
